...
def prepare_dataset(buildings_path: Path, gpt_features_path: Path | None = None) -> pd.DataFrame:
    ...
    if gpt_features_path is not None:
        try:
            ...
            LOGGER.warning("GPT features file '%s' not found; continuing without additional features", gpt_features_path)
        except ValueError as exc:
            LOGGER.warning("%s; continuing without GPT features", exc)
        else:
            dataset = dataset.join(gpt_features, how="left")
            LOGGER.info("Joined %d GPT feature columns from '%s'", gpt_features.shape[1], gpt_features_path)
    ...
    for col in numeric_cols:
        series = dataset[col]
        dataset[col] = series.fillna(0.0 if series.isna().all() else series.median())
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    dataset = prepare_dataset(args.buildings, args.gpt_features)
    LOGGER.info(
    ...
    LOGGER.info("Mean R^2 over %d folds: %.3f ± %.3f", len(scores), mean_score, std_score)
